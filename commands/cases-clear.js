...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('cases-clear')
    .setDescription('Clear ALL moderation cases for a user')
    ...
    .addStringOption(option =>
      option.setName('reason')
        .setDescription('Reason for clearing cases')
        .setRequired(false)
    ),
  ...
  async execute(interaction) {
    ...
    const userCases = Object.entries(cases).filter(
      ([_, data]) => data.userId === user.id
    );
    ...
    if (logChannel) {
      const logEmbed = new EmbedBuilder()
        .setTitle('Moderation Action: CASES CLEARED')
        .setColor(0x6d1f2e)
        .addFields(
          { name: 'User', value: `${user.tag} (${user.id})`, inline: false },
          { name: 'Cleared By', value: interaction.user.tag, inline: false },
          { name: 'Cases Removed', value: `${userCases.length}`, inline: true },
    ...
    const confirmEmbed = new EmbedBuilder()
      ...
      .addFields(
        ...
        { name: 'Removed', value: `${userCases.length}`, inline: true },
        { name: 'Reason', value: reason, inline: false }
      )
      .setTimestamp();
